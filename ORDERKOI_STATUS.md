...
**Current state:** fully working product on dev servers, on PostgreSQL 17. Backend tests 193/193 (run on PostgreSQL, `orderkoi_test` database). Playwright E2E smoke 5/5 (`frontend/e2e/smoke.spec.js` — dev servers must be running: `npx playwright test`). Frontend build + lint clean. Gmail SMTP live (app password in `backend/.env`, sends are background now). All business values (prices, free allowance, bKash, support email) env-driven via the public config endpoint. Admin endpoints scale-hardened for 500k users (2026-09-12 — see Phase 8e row); remaining pre-Docker scale work: missing indexes + `Numeric(12,2)`.

**Agreed launch order (revised 2026-09-06):** ① pre-deploy code fixes (DONE — see hardening batch above) → ② shift to PostgreSQL (DONE 2026-09-07 — see Phase 8b row) → ③ Dockerize + deploy after ② is verified. **② is verified; ③ starts next session.**
...
## 🔴 Phase 8c — Dockerize + deploy (NEXT — starting next session)
...
- [ ] DB indexes before/with 8c: `orders(created_at)`, `orders(seller_id, created_at)`, `sellers(created_at)` — new Alembic revision
- [ ] `Float` → `Numeric(12,2)` money columns (still deferred — exact taka)
