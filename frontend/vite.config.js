...
export default defineConfig(({ command, mode }) => {
  ...
  //
  // `vite build` only — `npm run dev` keeps falling back to the local
  // API so development needs no .env file at all.
  const env = loadEnv(mode, process.cwd(), 'VITE_')
  if (command === 'build' && !env.VITE_API_URL) {
    throw new Error(
      'VITE_API_URL is not set. It is baked into the bundle at build time, so ' +
        'this cannot be fixed after the build — set it in .env (compose passes ' +
        'it as a build arg) to the public API origin, e.g. https://api.orderkoi.example.com'
    )
  }
